...
export class TypeOrmChatRepository implements IChatRoomRepository {
    ...
    async findChatRoomById(id: string): Promise<ChatRoom> {
        ...
        return ChatRoom.create(chatRoomEntity.id, chatRoomEntity.name, chatRoomEntity.usersId);
    }
    generateId(): string {
        return '';
    }
    async saveChatRoom(chatRoom: ChatRoom): Promise<ChatRoom> {
        try{
            ...
            console.debug('[TypeOrmChatRepository][saveChatRoom] chatRoomEntity:', chatRoomEntity);
